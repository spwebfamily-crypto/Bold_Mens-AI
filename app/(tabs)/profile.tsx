...
function Row({ title, icon }: { title: string; icon: React.ReactNode }) {
  return (
    <Pressable className="flex-row items-center gap-3 border-b border-bmGold/10 py-4">
      {icon}
      <Text className="flex-1 text-base text-bmWhite">{title}</Text>
    ...
    <SafeAreaView className="flex-1 bg-bmBlack" edges={['top']}>
      <ScrollView contentContainerClassName="px-5 py-5">
        <View className="items-center">
          ...
          <Text className="mt-4 text-2xl font-bold text-bmWhite">{user?.name ?? 'BoldMens User'}</Text>
          <Text className="mt-1 text-sm text-bmDim">{user?.email ?? 'Sem email'}</Text>
          <View className="mt-3">
        ...
        <View className="mt-8 rounded-lg border border-bmGold/15 bg-bmDark p-4">
          <Text className="text-xs font-bold uppercase tracking-widest text-bmGold">O teu plano</Text>
          <Text className="mt-3 text-lg font-semibold text-bmWhite">
            {user?.plan === 'plus' ? 'Analises ilimitadas' : '3 analises por dia'}
          </Text>
          <Text className="mt-1 text-sm text-bmDim">{limit.label}</Text>
          {user?.plan === 'plus' ? (
        ...
        <View className="mt-6">
          <Text className="mb-1 text-xs font-bold uppercase tracking-widest text-bmGold">Conta</Text>
